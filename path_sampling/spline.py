...
class SplineGenerator(object):
    ...
    def __init__(self, gp_handler: GlobalPathHandler, ego_pose: Pose,
    ...
    def generate_lat(self, n: int, pts_per_spline: int, obs_pose: Pose,
        ...
        cls_pt = self._gp_handler.get_closest_point(obs_pose)
        # Slope intercept data of perpendicular to global path at closest point
        ppd_line = self._gp_handler.get_perpendicular(cls_pt, 0)

        # Point of intersection of perpendicular with left track boundary
        p1 = intersection_line_cubic(ppd_line, self._coeffs_left)
        # Point of intersection of perpendicular with left track boundary
        p2 = intersection_line_cubic(ppd_line, self._coeffs_right)
        ...
        e0 = - ((p1.x - obs_pose.x) + ppd_line.m * (p1.y - obs_pose.y))
        e0 /= (p2.x - p1.x) + ppd_line.m * (p2.y - p1.y)
    ...
    def generate_long(self, n: int, pts_per_spline: int, obs_pose: Pose,
